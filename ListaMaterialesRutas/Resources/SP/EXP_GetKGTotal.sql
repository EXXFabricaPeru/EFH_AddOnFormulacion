CREATE FUNCTION EXP_GetKGTotal(in ItemCode NVARCHAR(50), in Factor double, in Merma int) 
 RETURNS KGCalculado Double 
 LANGUAGE SQLSCRIPT AS
 BEGIN
	Declare HaveLAM  int;
	Declare TipoForm  nvarchar(3);
    	...
    	INTO KGCalculado  
     	FROM(
		     SELECT DISTINCT		
				...
				*CASE WHEN(:Merma=1) THEN (1+IFNULL(FLIN."U_EXP_MERMA",0)/100) ELSE 1 END "KG"	,0 "LineId"		     	 
		     FROM OITM "ITM" 
		     JOIN "@EXP_OFRM" "FO0" ON ITM."U_EXP_FORM"=FO0."Code"
			...
			SELECT DISTINCT		
		     	...
		     	(IFNULL(MLIN."U_EXP_GCM33",1)*:Factor)* -- material
		     	(FO0."U_EXP_QTT"*1000) --CantidadBase
				*CASE WHEN(:Merma=1) THEN (1+IFNULL(FLIN."U_EXP_MERMA",0)/100) ELSE 1 END "KG"	,FLIN."LineId"	 "LineId"		     	 
		     FROM OITM "ITM" 
		     JOIN "@EXP_OFRM" "FO0" ON ITM."U_EXP_FORM"=FO0."Code"
		     JOIN "@EXP_FFRUTA" "RUT"ON   RUT."Code"=FO0."Code"
    		 --LEFT  JOIN "@EXP_LAMINA" "FLIN" ON FLIN."Code"=FO0."Code"  --OJO
    		 LEFT  JOIN "@EXP_FREXTR" "FLIN" ON FLIN."Code"=FO0."Code"  --OJO
	...
	ELSEIF(:TipoForm='PK') THEN	-- PESO KILOS
       ...
       FROM( SELECT "FACTOR"/ SUM("FACTOR") OVER (PARTITION BY "AUX" ) *:KGCalculado *"MERMA"  "A"
