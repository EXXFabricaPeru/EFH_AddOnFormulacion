CREATE FUNCTION EXP_GetKGArti(in ItemCode NVARCHAR(50), in Factor double, in Merma int, in IsRuta int, iN mat  NVARCHAR(50)) 
 ...
 BEGIN
	...
	IF(:TipoForm='CM')THEN --CANT. MILLARES
    	...
    	INTO KGCalculado  
     	FROM(
		     ...
		     WHERE "ItemCode"=:ItemCode AND FLIN."U_EXP_MPRIMA" IS NOT NULL AND RUT."U_EXP_CODRUT" IN ('REB') 
		     and  RUT."LineId" =:IsRuta 
		     UNION 	
		     		     SELECT DISTINCT		
		     	:AnchoPT*--ANCHO PT
		     	:LargoPT*--LARGO PT
		     ...
		     LEFT JOIN "@EXP_TIPMAT" "MLIN" ON FLIN."U_EXP_MTRL"= MLIN."Name"
		     WHERE "ItemCode"=:ItemCode AND FLIN."U_EXP_MPRIMA" IS NOT NULL AND RUT."U_EXP_CODRUT" IN ('HAB') 
		     and  RUT."LineId" =:IsRuta 
		     UNION 	
		     		     SELECT DISTINCT		
		     	:AnchoPT*--ANCHO PT
		     ...
		     SELECT DISTINCT		
		     	...
		     	*CASE WHEN(:Merma=1) THEN (1+IFNULL(FLIN."U_EXP_MERMA",0)/100) ELSE 1 END 
		     	* (case when :mat='1' then 1 else 0 end)) -- material
				...
				* (case when :mat='2' then 1 else 0 end))
				"KG"		        	 
		     FROM OITM "ITM" 
		     JOIN "@EXP_OFRM" "FO0" ON ITM."U_EXP_FORM"=FO0."Code"
		     JOIN "@EXP_FFRUTA" "RUT"ON   RUT."Code"=FO0."Code"
     	...
     	FROM(
		     ...
		     SELECT DISTINCT		
				...
				+case when( FLIN."U_EXP_MPRIM2" IS NOT NULL) THEN ((FLIN."U_EXP_ESPESM2"*"EXP_GetConversion"(FLIN."U_EXP_UNESPM2"))* -- ESPE ETAPA
		     	(IFNULL(MLIN2."U_EXP_GCM33",1)*:Factor)) ELSE 0 END -- material
				*CASE WHEN(:Merma=1) THEN (1+IFNULL(FLIN."U_EXP_MERMAM2",0)/100) ELSE 1 END 
				* (case when :mat='1' then 2 else 0 end))
				"KG"		   	     	 
		     FROM OITM "ITM" 
		     JOIN "@EXP_OFRM" "FO0" ON ITM."U_EXP_FORM"=FO0."Code"
		     JOIN "@EXP_FFRUTA" "RUT"ON   RUT."Code"=FO0."Code"
	...
	ELSEIF(:TipoForm='PK') THEN	-- PESO KILOS
 		IF(:Merma=1) THEN		
       SELECT SUM("A")  INTO KGCalculado   
       FROM( SELECT "FACTOR"/ SUM("FACTOR") OVER (PARTITION BY "AUX" ) *:KGCalculado *"MERMA"  "A","RUT","EXT", "MAT"
     ...
     SELECT DISTINCT		
			...
			RUT."LineId" "RUT",
			FEXTR."U_EXP_CODRUTDE" "EXT",
			(IFNULL(FEXTR."U_EXP_ESPESOR",FLIN."U_EXP_ESPESM2")*"EXP_GetConversion"(IFNULL(FEXTR."U_EXP_UNESP",FLIN."U_EXP_UNESPM2")))* 
		     	(IFNULL(IFNULL(MLEX."U_EXP_GCM33",MLIN."U_EXP_GCM33"),1)*:Factor) "FACTOR",
			(1+IFNULL(IFNULL(FEXTR."U_EXP_MERMA",FLIN."U_EXP_MERMAM2"),0)/100) "MERMA"
			,'2' "MAT"
		     FROM OITM "ITM" 
		     JOIN "@EXP_OFRM" "FO0" ON ITM."U_EXP_FORM"=FO0."Code"
		     JOIN "@EXP_FFRUTA" "RUT"ON   RUT."Code"=FO0."Code"		     
    		  JOIN "@EXP_LAMINA" "FLIN" ON FLIN."Code"=FO0."Code"   and FLIN."U_EXP_CODRUT"=RUT."LineId"
			 LEFT JOIN "@EXP_FREXTR" "FEXTR" ON FEXTR."Code"=FO0."Code" AND FLIN."U_EXP_CODRUT" =FEXTR."U_EXP_CDRTDE"
	     	 LEFT JOIN "@EXP_TIPMAT" "MLIN" ON FLIN."U_EXP_MTRLM2"= MLIN."Name"			  
		     LEFT JOIN "@EXP_TIPMAT" "MLEX" ON FEXTR."U_EXP_MATE"= MLEX."Name"
		     WHERE "ItemCode"=:ItemCode AND ((FLIN."U_EXP_MPRIMA" IS NOT NULL) OR FEXTR."U_EXP_CDRTDE" IS NOT NULL) AND RUT."U_EXP_CODRUT" IN ('LAM') and FLIN."U_EXP_MPRIM2" IS NOT NULL
		     
		     )) WHERE ("RUT"=:IsRuta  OR "EXT" =:IsRuta ) AND "MAT"=:mat ;
